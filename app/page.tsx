...
export default function Home() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setMenuOpen((open) => {
  ...
  useEffect(() => {
    ...
    if (menuOpen) requestAnimationFrame(() => menuRef.current?.querySelector<HTMLAnchorElement>("a")?.focus());
  ...
  return <main ref={mainRef}>
    {introOpen && <div className="intro" ref={introRef} role="dialog" aria-label="Introdução AURUM by BYD">
      ...
      <button className="intro-skip" onClick={skipIntro}>Pular intro <ArrowUpRight aria-hidden="true" /></button>
    </div>}
    ...
    <button className="back-to-top" type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Voltar ao topo">
      <span className="back-to-top-ring" aria-hidden="true"/><ArrowUp aria-hidden="true"/><small>TOPO</small>
    </button>
    <header className="nav-shell"><a href="#top" className="nav-brand"><BrandMark compact /></a><nav aria-label="Navegação principal">{menu.map(([href,label]) => <a key={href} href={href}>{label}</a>)}</nav><button ref={menuButtonRef} className={`menu-button ${menuOpen ? "open" : ""}`} onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? "Fechar menu" : "Abrir menu"} aria-expanded={menuOpen} aria-controls="menu-principal"><span/><span/></button></header>
    <div ref={menuRef} id="menu-principal" className={`menu-overlay ${menuOpen ? "open" : ""}`} role="dialog" aria-modal={menuOpen || undefined} aria-hidden={!menuOpen}>{menu.map(([href,label],i) => <a key={href} href={href} tabIndex={menuOpen ? 0 : -1} style={{transitionDelay:`${100+i*70}ms`}} onClick={() => setMenuOpen(false)}>{label}</a>)}</div>

    <section className="hero" id="top"><div className="hero-image" aria-hidden="true"/><div className="hero-shade" aria-hidden="true"/><div className="red-orbit" aria-hidden="true"/><div className="hero-content"><p className="kicker">Apresentação confidencial · Visão de sociedade</p><h1><span>O futuro da</span><span>experiência</span><span className="gold">cinematográfica</span><span>começa aqui.</span></h1><p className="hero-copy">Uma aliança entre cultura, mobilidade e tecnologia para construir o cinema premium mais desejado da região.</p><a className="magnetic-cta" href="#visao"><span>Entrar na visão</span><ArrowDownRight aria-hidden="true" /></a></div><div className="hero-index"><span>01</span><i/><small>16</small></div></section>
    ...
    <footer><img className="footer-au" src="/brand/aurum-au.svg" alt="AU"/><p className="section-number">Próximo capítulo</p><h2>Vamos construir o cinema<br/>do <em>futuro.</em></h2><a className="magnetic-cta light-cta" href="mailto:contato@aurumcinema.com.br"><span>Iniciar a conversa</span><ArrowUpRight aria-hidden="true" /></a><div className="footer-row"><BrandMark compact/><span>Centro Cultural Oscar Niemeyer · Goiânia, GO</span><span>Apresentação confidencial · 2026</span></div></footer>
  </main>;
}
